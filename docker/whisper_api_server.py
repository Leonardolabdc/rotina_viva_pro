...
def _whisper_vad_options() -> tuple[bool, dict | None]:
    """
    VAD strips silence/noise so Whisper does not hallucinate repeating phrases on hiss.
    WHISPER_VAD_FILTER:
      - off / false — no VAD (risks loops like "o que é o que é…" on background noise).
      - strict / true — Silero defaults (can drop very quiet Portuguese).
      - relaxed — VAD on, threshold mais baixo e pausas mais curtas (recomendado no chat).
    """
    v = os.environ.get("WHISPER_VAD_FILTER", "relaxed").strip().lower()
    if v in ("0", "false", "no", "off"):
        return False, None
    if v in ("strict", "1", "yes", "on", "true"):
        return True, None
    return True, {
        "threshold": 0.25,
...
def _whisper_condition_on_previous_text() -> bool:
    """
    If False (default), evita a cadeia janela-a-janela que causa loops de repetição
    (ver documentação faster-whisper em condition_on_previous_text).
    """
    v = os.environ.get("WHISPER_CONDITION_ON_PREVIOUS_TEXT", "").strip().lower()
...
async def transcribe(
    ...
    try:
        ...
        use_vad, vad_params = _whisper_vad_options()
        cond_prev = _whisper_condition_on_previous_text()
        try:
            ns_thresh = float(os.environ.get("WHISPER_NO_SPEECH_THRESHOLD", "0.42"))
        except ValueError:
            ns_thresh = 0.42
        ...
        def _transcribe_pass(
            *,
            ...
            kw: dict = {
                ...
                "no_speech_threshold": no_speech if no_speech is not None else ns_thresh,
            }
            if extra:
                kw.update(extra)
            return _model.transcribe(tmp_path, **kw)

        segments_gen, info = _transcribe_pass(
            vad=use_vad,
            vad_p=vad_params,
            lang_use=lang,
            beam=_beam_size,
        ...
        probe = _segments_text(segments)

        _retry_raw = os.environ.get("WHISPER_EMPTY_RETRY", "true").strip().lower()
        _do_retry = _retry_raw in ("1", "true", "yes", "")
        if not probe and use_vad and _do_retry:
            logger.info(
                "Empty transcript with VAD; retry without VAD "
                "(beam=1, no_repeat_ngram, repetition_penalty)"
            )
            segments_gen, info = _transcribe_pass(
                vad=False,
                vad_p=None,
        ...
        if not probe and _do_retry and lang is not None:
            logger.info("Empty transcript; retry with language autodetect (no VAD)")
            segments_gen, info = _transcribe_pass(
                vad=False,
                vad_p=None,
                lang_use=None,
